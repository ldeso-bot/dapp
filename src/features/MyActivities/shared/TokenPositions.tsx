...
export const TokenPositions: FC<TokenPositionsProps> = ({
  ...
  return (
    <div className="border-t border-border-subtle pb-5">
...
isPartiallyClaimed: ${lock.isPartiallyClaimed}
  ...
  return (
    <div title={title} className="rounded-xl overflow-hidden">
      {/* Status and date */}
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 px-3 py-2 border-border-default border-x-1 border-t-1 rounded-t-xl bg-surface-3">
        <div
          className={cn(
            'flex flex-row gap-1 px-2 py-1 rounded-3xl font-bold',
            isMatured
              ? 'bg-positive-bg text-text-highlight'
      ...
      {/* Content */}
      <div className="grid gap-4 px-3 py-3 border-border-default border-x-1 border-b-1 rounded-b-xl grid-cols-1 sm:grid-cols-12">
        {/* Principal */}
        ...
        <div className="flex flex-col gap-2 sm:col-span-5 min-w-0">
          ...
          <div className="flex items-center justify-between sm:justify-start gap-2 min-w-0">
            <div className="bg-surface-2 text-text-1 py-1 px-2 rounded-full text-size-12 font-medium shrink-0">
              kVCM
            </div>

            <div className="text-size-14 text-text-1 font-[400] text-right sm:text-left break-words min-w-0">
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex sm:justify-end sm:gap-2">
            <Button
              ...
              className={cn(
                ...
                {
                  'bg-surface-2 text-text-3 hover:bg-surface-2':
            ...
            {showTopUp && (
              <Button
                colors="neutral"
                className="text-size-12 w-full text-text-1 col-start-2"
                onClick={() => {
                  if (lock.token === 'k2') {
