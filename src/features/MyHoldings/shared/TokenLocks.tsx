...
type TokenLocksProps = {
  isOpen?: boolean;
  onOpenChange?: (isOpen: boolean) => void;
};

export const TokenLocks: FC<TokenLocksProps> = ({ isOpen, onOpenChange }) => {
  ...
  return (
    <Accordion
      type="single"
      collapsible
      value={isOpen ? 'locks' : undefined}
      onValueChange={(value) => onOpenChange?.(value === 'locks')}
    >
        ...
        <AccordionTrigger className="hover:no-underline border-t border-gray-100 flex items-center justify-start rounded-none gap-1">
          <div className="flex items-center gap-2">
            <div className="text-size-14 text-gray-900 font-[400]">
              View locks ({totalLocks})
            </div>
            <Tooltip content="Locks tooltip here..." />
          </div>
        </AccordionTrigger>
