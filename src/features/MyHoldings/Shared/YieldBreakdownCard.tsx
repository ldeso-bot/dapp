...
export default function YieldBreakdownCard({ amount, selectedMaturity }: Props) {
  // todo - replace with actual calculations
  const totalYield = selectedMaturity.yieldPercent ?? 3 + (selectedMaturity.incentivesYield ?? 0);
  ...
  const incentivesYieldFormatted = (selectedMaturity.incentivesYield ?? 0 * amount).toFixed(2);
    ...
    <>
      ...
      <section className="grid grid-cols-1 gap-4">
        ...
        <div className="p-4 bg-void-80 rounded-xl text-center">
          <h3 className="text-size-14 font-medium text-void-20 mb-1">Total Value at Maturity</h3>
          <p className="text-white font-semibold text-size-18">
            {totalYieldFormatted} kVCM
          </p>
          <p className="text-void-20 mt-1">
            + 0 K2 tokens
          </p>
        </div>
      </section>
